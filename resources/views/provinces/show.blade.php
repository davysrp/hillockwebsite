@extends('adminlte::page')

@section('content')

<div class="panel panel-default">
    <div class="panel-heading clearfix">

        <span class="pull-left">
            <h4 class="mt-5 mb-5">{{ isset($province->name) ? $province->name : 'Province' }}</h4>
        </span>

        <div class="pull-right">

            <form method="POST" action="{!! route('provinces.province.destroy', $province->id) !!}" accept-charset="UTF-8">
            <input name="_method" value="DELETE" type="hidden">
            {{ csrf_field() }}
                <div class="btn-group btn-group-sm" role="group">
                    <a href="{{ route('provinces.province.index') }}" class="btn btn-primary" title="Show All Province">
                        <span class="glyphicon glyphicon-th-list" aria-hidden="true"></span>
                    </a>

                    <a href="{{ route('provinces.province.create') }}" class="btn btn-success" title="Create New Province">
                        <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                    </a>

                    <a href="{{ route('provinces.province.edit', $province->id ) }}" class="btn btn-primary" title="Edit Province">
                        <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
                    </a>

                    <button type="submit" class="btn btn-danger" title="Delete Province" onclick="return confirm(&quot;Click Ok to delete Province.?&quot;)">
                        <span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
                    </button>
                </div>
            </form>

        </div>

    </div>

    <div class="panel-body">
        <dl class="dl-horizontal">
            <dt>Name Kh</dt>
            <dd>{{ $province->name_kh }}</dd>
            <dt>Name</dt>
            <dd>{{ $province->name }}</dd>
            <dt>Created At</dt>
            <dd>{{ $province->created_at }}</dd>
            <dt>Updated At</dt>
            <dd>{{ $province->updated_at }}</dd>

        </dl>

    </div>
</div>

@endsection
